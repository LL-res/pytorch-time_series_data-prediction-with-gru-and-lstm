...
def train(metrics,type):
    """
        metrics : list ,训练的未经拆分的原始数据
        type : str ,metric的类型，用于保存对应的模型
    """
    train_features, train_target = data_prediction_to_f_and_t(metrics, lengths, targets)
    ...
    end_gru = time.perf_counter()
    print('gru训练时间为：{:.2f}s'.format(end_gru-start_gru))
    torch.save(net_gru.state_dict(), 'gru_{type}.pt'.format(type=type))